...
class SettingsCheck extends Component {
  ...
  findDevices() {
    ...
    this.time = this.device.time_status;
    console.log(this.device.time_status);
  }
        ...
        {Alert.alert(this.props.screen.status, this.props.screen.check, [
          ...
          },
          {
            ...
            onPress: () => {
              Platform.OS === 'ios' &&
                this.sendMessageIOS(
                  `${this.prefix}${this.password}${this.time}`,
                  this.phoneNumber,
                );
              Platform.OS === 'android' &&
...
const mapStateToProps = (state) => {
  return {
    screen: state.screens.settings_time_system[state.currentLanguage],
